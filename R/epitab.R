  ...
  function(x, y = NULL,
    ...
    if(method=="oddsratio"){
      ...
      tab <- cbind(rr$x[,1],rr$p.exp[-nrow(rr$p.exp),1],
                   rr$x[,2],rr$p.exp[-nrow(rr$p.exp),2],
                   rr$measure, pval)
      cn <- colnames(x)
      rownames(tab)<-rownames(x)
      colnames(tab)<-c(cn[1], "p0", cn[2], "p1",
      ...
      if(verbose){
        fin <- list(tab = tab,
                    ...
                    x = rr$x,
                    data = rr$data,
                    p.exposed = rr$p.exposed,
                    p.outcome = rr$p.outcome,
                    p.value = rr$p.value,
                    correction = correction
                    )
      } else {
    ...
    if(method=="riskratio"){
      ...
      tab <- cbind(rr$x[,1],rr$p.out[-nrow(rr$p.out),1],
                   rr$x[,2],rr$p.out[-nrow(rr$p.out),2],
                   rr$measure, pval)
      cn <- colnames(x)
      rownames(tab)<-rownames(x)
      colnames(tab)<-c(cn[1], "p0", cn[2], "p1",
    ...
    if(method=="rateratio"){
      ...
      cn <- colnames(x)
      rownames(tab)<-rownames(x)
      colnames(tab)<-c(cn[1:2], "rateratio", "lower", "upper", "p.value")
      if(!is.null(names(dimnames(x)))){
        names(dimnames(tab)) <- names(dimnames(x))
      }
      if(verbose){
